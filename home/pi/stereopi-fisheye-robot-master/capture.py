...
# Camera settings (Capture Resolution)
cam_width = 1280
cam_height = 480
frame_rate = 20

# **🔹 Adjustable Preview Resolution (does NOT affect saved images)**
preview_width = 640   # Set desired preview width
preview_height = 300  # Set desired preview height

# Adjust resolution to match PiCamera constraints
cam_width = int((cam_width + 31) / 32) * 32
cam_height = int((cam_height + 15) / 16) * 16

print(f"Camera capture resolution: {cam_width} x {cam_height}")
print(f"Camera preview resolution: {preview_width} x {preview_height}")

# Initialize stereo camera
camera = PiCamera(stereo_mode='side-by-side', stereo_decimate=False)
camera.resolution = (cam_width, cam_height)
camera.framerate = frame_rate
camera.hflip = True  # Swap left and right cameras
camera.vflip = True
...
try:
    while True:
        ...
        # **🔹 Resize the preview to `preview_width` x `preview_height`**
        preview_frame = cv2.resize(frame, (preview_width, preview_height), interpolation=cv2.INTER_LINEAR)

        # Display resized preview (NO Full-Screen Mode)
        cv2.imshow("Stereo Preview", preview_frame)
        ...
        if GPIO.input(BUTTON_PIN) == GPIO.LOW:  # Button pressed
            ...
            # Save the captured frame (full resolution)
